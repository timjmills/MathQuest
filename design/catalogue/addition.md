...
`design/research/operations-facts-v2.md` is the P4 content specification. It supersedes the AD-F / AD-U / AD-C / AD-V / AD-S sketches above where the two differ, and it adds per-step scaffold classes, written responses, `Say:` frames, a misconception bank with wrong-answer rules, and a density check. Corrections to this family's verdicts arising from it:

- **`add_10_regroup` is the bridging-ten skill** (spec §7). Its label changes; its id and code do not. AD-F9 is right and is now fully specified. **Shipped 2026-09-20** as "Add — Bridging Ten (sums 11–18)": 36 members, all single-digit addends, all sums 11–18, verified correct on a 20-item dump. The gate's `variety` note on it is a false positive — 36 is the complete set.
- **AD-F15 bundles two changes** — the constant growing past ten *and* the band moving to 30. The spec splits the band rise into its own rung (AF-10) and derives the band from the constant everywhere else (spec §2.2, Q3).
- ~~**Nine `number_families_*` / `mult_chart_*` ids produce 1 distinct item in 200.** Their verdict is REDO before MERGE.~~ **WITHDRAWN by independent review, 2026-09-20.** That figure was an artefact of the old gate hashing `q.text`, which is a constant string for these ids while the item itself lives in `q.visual`. Items generated directly are varied and correct — `number_families_add` gives `2,7,9` / `10,7,17` / `8,5,13` / `9,1,10`, `mult_chart_hard` gives 20 different 22-cell charts — and `git diff js/modules/gen-operations.js` contains no change to either family, so nothing was "fixed" in between. The verdict returns to **MERGE** (one `practiceLevel`, P-AT-9). The one real residue is that `number_families_mult` is capped at factors 2–5 (16 distinct in 240); widen it during the merge.
- **The missing-operator branch has exactly one legal home**, spec §18 XD-7, with a written sign as the response. It is removed from `add` and from the `mixed_*` pools rather than being fixed in place.
- **Ragged lengths come before equal lengths** (AC-3 before AC-4), which `generateAddPair`'s `minVal = Math.max(2, Math.floor(maxVal / 10))` currently forbids.
