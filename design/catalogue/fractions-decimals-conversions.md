...
| FR-F5 | Bigger Numbers, Same Method | range | `fraction_of_set_nv` opt `level=hard` (alias of both `*_hard` ids) |
...
| `fraction_of_set_hard` | 4 | visual-grid | schema-story | merge → `fractions:fraction_of_set` | twin: same code path (:3848), differing only in the denominator pool (:3873) and a 60% "missing numerator" branch (:3883); same emoji branch | alias option `level=hard`; the missing-numerator branch becomes NEW `frac_of_set_missing` |
...
| `fraction_of_set_hard_nv` | 4 | equation-drill | schema-story | merge → `fractions:fraction_of_set_nv` | twin (:1996 / :2045); the only real differences are the number pool and the "3/5 of a number is 9" type | alias option `level=hard`; the find-the-whole type becomes NEW `frac_of_set_whole` |
...
**`fraction_of_set`, `fraction_of_set_hard` (split, merge).** One id with a picture and one without is the right pair; a second pair that differs only in number size is not. `fraction_of_set_hard` becomes `level=hard` on `fraction_of_set`, and the same for the two `_nv` ids. Drop the emoji objects for the plain counters the print path already draws, remove the "5 equal groups of 6 · 30 objects total" caption (it is the working), and add the ring-the-groups notate step. The missing-numerator and find-the-whole branches become their own late steps.
...
**`identify_nv` (split).** Three types, one of which quietly demands simplest form. Split into "write the fraction from the two words", "write the fraction from a sentence" and a story step, and state the simplest-form rule in the prompt wherever it applies.
...
**`f_to_d`, `d_to_f`, `f_to_p`, `p_to_f` (split).** All four share a 30% drag-to-bin branch that is a sort page, not a practice item, and all four print their method inside the cell. `d_to_f` must either say "in simplest form" or accept 2/10. `p_to_f` draws from five percents only and repeats within one page.
...
2. **Simplest form.** Today `identify`, `identify_nv`, `d_to_f` and `percent_visual` silently require the simplest form while `write_fraction` silently forbids it, and `identify` offers the equivalent unsimplified fraction as a wrong option. Recommendation: the model answer is always the counted or converted fraction; simplest form is required only when the prompt says "in simplest form", and equivalent answers are accepted everywhere else.
