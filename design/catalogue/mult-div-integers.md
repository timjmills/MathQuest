...
`design/research/operations-facts-v2.md` is the P4 content specification. It supersedes the MU-A / MU-B / DV-A / DV-B / DV-C / WP-M sketches above where the two differ. Corrections to this family's verdicts arising from it:

- **The division fact-set order cannot start with {0, 1, 2, 5, 10} literally**, because `n ÷ 0` is undefined. Read the first block as {1, 2, 5, 10} with `0 ÷ n` as an edge item at DV-A10, and keep `÷ 5` ahead of `÷ 2` because 5 is the tally exemplar of L-7 (spec §15, Q8).
- ~~**`multiply` and `divide` are REDO, not FIX.**~~ **CLOSED 2026-09-20.** Both are single-operation generators now: 20 items from `multiply` are all `a × b` (`6×8=48`, `7×7=49`, `12×12=144`) and 20 from `divide` are all `a ÷ b` (`48÷6=8`, `144÷12=12`), every answer correct, no multiple choice. **One residual defect**, new and unlisted: at Max Number 100 both emit answers *above the band* — `12 × 12 = 144`, `11 × 12 = 132`, `132 ÷ 11`, `120 ÷ 10` — because they behave as 1–12 table drills while their names promise nothing about a table. Either make the band bind (P-35) or rename them.
- **`long_div_2digit` produces 15 distinct items in 240 — re-confirmed, still broken.** The divisor is drawn from **{11, 12} only**; every dividend is an exact multiple and every quotient a single digit (`77÷11`, `72÷12`, `96÷12`, `24÷12`). A pupil "dividing by 2-digit numbers" never meets 13, 15, 24 or 25. DV-C10 and DV-C11 cannot be delivered as options on it until the generator is rebuilt.
- **MU-B7 (the placeholder zero) is load-bearing.** It is a `notation`-scope rung of its own, not a hint inside MU-B8, because M-M6 (the omitted placeholder zero) is the single biggest 2 × 2 error.
- **Density:** a 2 × 2 multiplication cell and every long-division cell are long procedures and must declare **4 items per page at every size**, not 6 (spec §21).
