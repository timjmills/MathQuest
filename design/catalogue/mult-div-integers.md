...
1. **Fact sets are an option, not skills.** The owner's {0,1,2,5,10} → {3,4,6} → {7,8,9} → {11,12} order cannot be expressed today because no fact skill takes a set. Recommendation: add `facts: [n] | {from, to}` as an option on `mult_facts` and `div_facts` (and their `+` / `−` twins) rather than creating 24 new skill ids; the ladder steps then differ only by the option value, which is exactly one delta per step.
2. **`mult_facts` and `div_facts` change identity when the teacher raises Max Number.** At range 500 or above they become column multiplication and long division. Recommendation: facts ignore `state.range` entirely (the fact set is the range), and the print dialog shows a note when a fact section is chosen with a large Max Number.
3. **Two partial-quotient representations.** `box_division_easy/hard` and `area_model_div_2by1/3by1` teach the same optional second strategy with different drawings. Recommendation: keep both ids (share codes), make the box the default drawing for the DV-C' ladder and list the area model as the same step's alternative representation, so a teacher never gets both shapes in one section.
