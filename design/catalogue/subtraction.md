...
| F17 | Facts to 20, Mixed | range (the band, not the constants) | `sub_20_mixed`, `sub_facts` opt `facts='all'`, band 20 |
| F17a | Subtract 11, 12, 13 | range (fact set) | `sub_facts` opt `facts={from:11,to:13}`, band 30 (owner ruling: + and − facts run to 30) |
...
**`sub_facts` (redo).** Three separate faults. The pair constraint makes the regrouping half of the fact table unreachable; the fact set cannot be selected, so the family's whole teaching order (Subtracting 1 … 10, then 0) cannot be printed; and orientation flips per item, which breaks P-16 and P-28 on one page. Rebuild around a fact-set descriptor (`facts:[n]`, `facts:{from,to}`, `facts:'all'`; the constant 0 to 13) and a band that caps the **minuend** and nothing else (ruled 2026-09-19: `a + b <= 20` is the wrong gate and is what makes `13 − 8` unreachable), a fixed orientation per section, the dot tile beside the subtrahend as cue part 1, and `n − 0` / `n − n` as the last set. The duplicate generator in `print-generate.js` (facts to 100) must be deleted, not fixed, so that print and screen come from one source.
...
**`sub_10_regroup` (redo).** There is no regrouping inside a single digit — the subtraction twin of the `add_10_regroup` case the owner ruled on (a band caps the minuend, so nothing within 10 can regroup); the generator can only satisfy its own gate with `10 − n`, which is why six of twelve items repeat. Rename the step to "Subtract From 10" (SU-F F14, the bridge into "use ten to subtract") and generate all nine facts plus `10 − 10` and `10 − 0`.
...
6. **RULED (owner, 2026-09-19): fact-set options for `sub_facts`.** The constant is an option, 0 to 13 ("Subtract 2"), with a mixed / cumulative setting, exposed as `facts:[n]` / `facts:{from,to}` / `facts:'all'` exactly as for × and ÷. The order is Subtracting 1, 2, 3 … 10, then 11, 12, 13, then 0 with `n − n` last. **Subtraction facts run to 30** — "facts to 12" was a × ÷ ruling — and the band caps the **minuend**, independently of the constant, so "Subtract 7" at band 20 draws minuends 7 to 20 (`PEDAGOGY_STANDARD.md` P-FL-20). This also settles the `a + b ≤ 20` defect in item 3 of the summary: the gate is on the minuend, never on the sum of the two operands, so `13 − 8` and `17 − 9` become generable.
...
## P4 cross-reference (2026-09-20)

`design/research/operations-facts-v2.md` is the P4 content specification. It supersedes the SU-F / SU-C / SU-R / SU-Z / SU-W / SU-U sketches above where the two differ. Corrections to this family's verdicts arising from it:

- ~~**`sub_10_regroup` is not the mirror of `add_10_regroup`** … the id should be renamed to "Subtract From 10" (nine members).~~ **OVERTAKEN BY CODE, 2026-09-20.** The operations wave shipped the opposite ruling: `sub_10_regroup` is now **"Subtract — Bridging Ten (minuends 11–18)"**, the exact mirror of `add_10_regroup` — 36 members, single-digit subtrahend, every item bridging (`12 − 7`, `18 − 9`, `13 − 9`, `11 − 8`), all 20 sampled answers correct. The independent review does not reopen this: a nine-item "Subtract From 10" set cannot fill a six-item page across a term, and bridging ten is the true counterpart of the addition rung that `PEDAGOGY_STANDARD.md` L-1 already defines. If the owner still wants `10 − n`, it is an **earlier, separate** rung, not a rename of this id. The audit message that prompted the original finding no longer fires.
- **SU-Z has no skill for any of its six rungs** and needs six appended ids. That collides with the "572 codes" gate as the brief states it; `ws-code-snapshot.mjs` is in fact append-only by design and its printed count is a live number (spec §22.4, Q4). This must be settled before SU-Z is built.
- **The notate page and the full page of an across-zeros rung are one rung, two pages**, not two rungs (spec §12). Written as two steps they break P-1 at every rung boundary.
- **M-S1 — subtracting the smaller digit from the larger regardless of position — is 31.2% of all multi-digit subtraction errors** in Nelson & Powell (2018). It is the reason SU-R R2 (decide-only) exists and it is the first distractor every error-analysis page in this family should use (spec §20.2).
