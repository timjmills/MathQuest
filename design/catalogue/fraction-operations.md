...
3. Worst problem 2, silent type mixing in almost every skill. A 25% "Click ALL ..." multi-select, a 20% drag-to-bins sort, and a 3-way variant rotation sit inside the computing skills. The sorts are estimation tasks ("sums greater than 3"), not the skill on the label. The rotation names are wrong in the code ('missing' means "with regrouping" at L1609 and L1664; 'missing_num' means "common denominator already shown" at L1475).
...
| `decompose_fractions` | 4 | visual-grid | hands-match | redo | answer tiles printed L665-667; n/n glut (4 of 6 in print baseline) from L636-637; typed sum cannot be marked (`answer-check.js:665`); only one decomposition type | A1 |
...
8. Answer conventions: simplest form required or not; mixed or improper accepted; slash or stacked in print.

## Questions for the owner

1. **Simplest form.** Today any equivalent answer is accepted, yet hints and some stems say "Simplify". Recommendation: accept any equivalent by default (CCSS does not require simplest form at Level 4), add a family option "simplest form" that changes the instruction and makes the check strict, and delete the 'simplify' variants that do not differ from 'straight'.
...
3. **Merge the twins.** 15 picture / no-picture pairs, 3 plain word pairs and `frac_as_div_word` become aliases with options `model: none` or `pictures: false`. Recommendation: yes; the picture is a scaffold level, and the ladders above depend on it. Old ids stay in place for share codes.
