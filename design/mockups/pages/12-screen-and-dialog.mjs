...
const feedback = frame({
    ...
    note: '<b>12-D · Feedback states (chrome, so green and red are allowed).</b> Left: a Guided item — unlabelled cell, grey regroup boxes, a live tick or cross as each digit is typed; the wrong 7 stays visible and focus has moved on. Middle: an Independent item before Check — no marks at all. Right: after Check — a 16 px badge hangs outside the bottom-right corner of every answered box with a 2 px outline, the right-most wrong box takes the focus ring, the ring tint goes on the card frame and the banner, and the paper stays white. Tick is a circle, cross is a square: shape carries the meaning. Regroup boxes and the empty leading box are never marked.',
    body: `<main class="ws-body p12-fb">
${fbPanel('Guided item', 'Live tick or cross per digit.', '', cell(sStack(546, 178, MINUS, { ans: ['', '', '7', '8'], rg: ['', '', '3', '16'], marks: ['', '', 'no', 'ok'], focus: 1, greyRg: true }), { cls: 'p12-solo' }), 'Marks show as you type.', '', actions({ checkOff: true }))}
...
const dialog = frame({
    ...
    note: '<b>12-E · Print dialog wireframe, 1100 px.</b> Left: the controls of one section (Sheet type with Lesson-packet parts as tick-boxes, Count, Columns Auto and 1–10 with its persistent clamp note — the stored choice 5 stays filled while the outlined 3 is what prints —, Size S / M / L, place-value labels, scaffold tick-boxes with a disabled box giving its reason, hints on tests, timing, objects; “More options” shown open: Look, Item labels, Problem mix) and the whole-job options (header fields, form and seed, photocopy-safe). Right: the live A4 thumbnail built from the real print DOM (3 × 2 = 6 items, the Independent ceiling at size L), the “Fits” line, a 1 / 3 pager with a page strip for the open section, a plain page count for the whole job, and Download PDF / Print. Flat product styling on the app’s purple; no gradients or glass.',
