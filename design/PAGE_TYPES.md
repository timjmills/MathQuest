...
Every option on every role lives in the print dialog. Scope is `job` (the whole print job) or `section` (one block of skills on a sheet). Remembered options persist between sessions.
...
- **PT-CMP-1. Governing rule.** Any skill can appear on any page role. A role never refuses a skill; the skill's `footprint` decides how many items fit, never whether the skill is allowed.
