...
| **Quiz** | **Build from** a CCSS domain or standard (`skillsMatchingCode`: `3.NBT`, `3.NBT.2`), an EE, a WRM year -> unit (block) -> small step (`skillsForWrmStep`), or a lesson: the tagged skills join with equal weights (editable; up to 12 per pick); skills (weights as Practice); versions Form A and / or Form B (B has new numbers); pages; columns; size. **Scoring**: 1 point each, by question type (a points value per skill), or per question (0.5 to 20); the total prints in the Score box. **Tags**: every question carries its skill (a hidden marker in its cell); the key gains a *score summary* page: each question's CCSS / EE / WRM tags and points, the total, and points per standard so the teacher sees which standards were missed. Standards never print on the pupil page (HD-6). | the **Practice engine** (`independent`, weighted, `tagItems`), one sheet per form (owner: "use the same practice paper engine"); `print-sheet.js` `scoreQuiz` adds the scores and the summary. The source and the scoring travel in the request (saved printouts reprint them); a share-code key for them needs a codec-registry range (not allocated yet) |
...
- **Old role ids decode to a paper** (`paperOfRole`, never break a saved set or a link): independent / more-practice /
  ...
  -> Quiz Form A / B (a stored `test` request still builds the old Test page); lesson / opener / pre-skill-check / scripted-model / guided -> Lesson with the matching part.
