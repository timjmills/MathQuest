...
(async () => {
    ...
    const { fact, stack, equation, TIMES, DIV } = kit;
    ...
    function problem(kind, p, size) {
        if (kind === 'fact') {
            const f = fact(p.a, p.b, p.op, { pt: DIGIT[size] });
            return `<div class="spec-fact" style="--fd:${DIGIT[size]}pt">${f.html}</div>`;
        }
        if (kind === 'stack') return stack(p.a, p.b, p.op, { heads: true });
        if (kind === 'round') {
            return `<div class="spec-round"><div style="font-size:var(--ws-text);font-weight:700;margin-bottom:1mm">Round to the nearest ${p.place}.</div>`
                + equation([p.n, '→', '_line'], size, String(p.n).length) + '</div>';
        }
    ...
    function cellFor(spec, size) {
        ...
        const html = attachPane(problem(kind, p, size), PANES[pane].draw(p, ctx), pl);
    ...
    // ---------------------------------------------------------------- screen twins
    const ALLSPECS = [...K2, ...PLACE, ...MODELS, ...EXTRAS.filter((s) => !s.ctx)];
    const seen = new Set();
    ...
    function screenCard(s, digitPx) {
        const p = s.p;
        const q = p.kind === 'round' ? `Round ${p.n} to the nearest ${p.place}.` : `${p.a} ${glyph[p.op]} ${p.b} =`;
...
`;
    const html = kit.doc('Support panes specimen', [...boards, screen(1280, 5.4, 56, 15), screen(390, 3.8, 40, 8)], css)
        .replace('../kit/sheet-kit.css', '../mockups/kit/sheet-kit.css');
