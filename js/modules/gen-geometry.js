...
export function generateGeometryQuestion(q, mappedSkill, helpers) {
    const { rng, range, applyDecimals, ensureTables } = helpers;
            ...
            if (geoSkill === "area_unit_squares") {
                // Build lane geometry: the kit shape-grid figure (gen-geo-kit.js).
                return areaUnitSquares(q);
            } else if (geoSkill === "perimeter_grid") {
                return perimeterGrid(q);
                ...
                q._variant = _perimVariant;
                if (_perimVariant !== 'word') return perimeterFigure(q, _perimVariant, maxDim);
                return storyFigure(q, 'perimeter', maxDim);
                ...
                const _areaVariant = (window.pickVariant
                    ...
                    : (Math.random() < 0.40 ? 'standard' : (Math.random() < 0.50 ? 'missing' : 'word')));
                q._variant = _areaVariant;
                // "Area Only" is grade 3 (3.MD.C.7): rectangles and squares. The half-base-times-height
                // triangle it used to deal is grade 6 content (options-r3 critic).
                if (_areaVariant !== 'word') return areaFigure(q, _areaVariant, maxDim);
                return storyFigure(q, 'area', maxDim);
                ...
                // Build lane geometry: right, acute or obtuse triangle with its dotted height (gen-geo-kit.js).
                return triangleFigure(q, Math.max(8, Math.min(20, Math.ceil(Math.sqrt(state.range)))));
            } else if (geoSkill === "area_polygon_decompose") {
                // Build lane geometry: an L-, T- or U-shape with its side lengths; the split line is the
                // Support-level-2 hint (it used to be shaded in on every item, so nothing was decomposed).
                return decomposeFigure(q, Math.max(6, Math.min(12, Math.ceil(Math.sqrt(state.range)))));
                ...
                // Build lane geometry: one labelled rectangle, "Perimeter = [ ]" and "Area = [ ]" in the cell.
                return areaPerimeterFigure(q, maxDim);
                ...
                q._variant = _ckind;
                return compositeFigure(q, _ckind, maxDim);
