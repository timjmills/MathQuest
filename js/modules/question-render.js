...
export function wireBoxValidation(visualAidEl, q) {
    ...
    const tryAdvance = () => {
        ...
        advanceTimer = setTimeout(() => {
            ...
            // Fire the FULL celebration suite (sfx + mascot cheer + XP burst +
            // confetti + correct-bg). Previously only some paths confettied,
            // and seq_fill / grid-fill widgets celebrated silently. The shared
            // helper below skips its own auto-advance — the per-mode routing
            // immediately after handles transition (MAP / submitAnswer / etc.).
            if (typeof window._celebrateCorrectAnswer === 'function') {
                window._celebrateCorrectAnswer({
                    xpText: '+10 XP',
                    skipAdvance: true,
                });
            }
...
function _handleMultiPlaceSubmit(opts) {
    ...
    if (allCorrect) {
        ...
        // skipAdvance: this branch schedules its own transition below
        // (it depends on shouldShowNextButton + MAP routing).
        if (typeof window._celebrateCorrectAnswer === 'function') {
            window._celebrateCorrectAnswer({
                xpText: `+${correctXP} XP`,
                skipAdvance: true,
...
export function renderQuestion() {
    ...
    if (!_calcVisible && typeof window !== 'undefined' && typeof window.hideCalculator === 'function') {
    ...
    if (isFactsColumnVisual) {
        ...
        const _formattedHtml = formatQuestionTextForScreen(q.text);
        questionTextEl.innerHTML = (typeof window !== 'undefined' && typeof window.linkVocabInText === 'function')
            ? window.linkVocabInText(_formattedHtml)
            : _formattedHtml;
...
export function checkOrderingAnswer() {
    ...
    if (isCorrect) {
        ...
        // Full celebration: confetti + correct-bg + sfx + mascot + XP burst.
        // skipAdvance: this path schedules its own transition based on
        // shouldShowNextButton + MAP routing.
        if (typeof window._celebrateCorrectAnswer === 'function') {
            window._celebrateCorrectAnswer({ xpText: '+10 XP', skipAdvance: true });
        } else {
            document.getElementById("questionCard").classList.add("correct-bg");
            confetti();
        }
...
export function checkExpandedAnswer() {
    ...
    if (isCorrect) {
        ...
        document.getElementById("gameScore").innerText = `${state.score} Correct`;
        // Full celebration suite. skipAdvance: path schedules own transition.
        if (typeof window._celebrateCorrectAnswer === 'function') {
            window._celebrateCorrectAnswer({ xpText: '+10 XP', skipAdvance: true });
        } else {
            document.getElementById("questionCard").classList.add("correct-bg");
...
export function checkAreaModelAnswer(input) {
    ...
    if (allFilled) {
        ...
        if (allCorrectOverall) {
            ...
            state.lastAnswerCorrect = true;
            document.getElementById("gameScore").innerText = `${state.score} Correct`;
            // Full celebration suite. skipAdvance: own MAP/transition routing.
            if (typeof window._celebrateCorrectAnswer === 'function') {
                window._celebrateCorrectAnswer({ xpText: '+20 XP', skipAdvance: true });
            } else {
                document.getElementById("questionCard").classList.add("correct-bg");
                confetti();
            }
...
export function checkNumberFamilyAnswer() {
    ...
    if (allFilled) {
        ...
        if (allCorrect) {
            ...
            state.lastAnswerCorrect = true;
            document.getElementById("gameScore").innerText = `${state.score} Correct`;
            // Full celebration. skipAdvance: own MAP / transition routing below.
            if (typeof window._celebrateCorrectAnswer === 'function') {
...
export function checkNumberLinePlacement() {
    ...
    if (isCorrect) {
        ...
        document.getElementById("gameScore").innerText = `${state.score} Correct`;
        // Full celebration suite (sfx + mascot + XP burst + confetti + flash).
        // skipAdvance: this path schedules its own transition based on MAP mode.
        if (typeof window._celebrateCorrectAnswer === 'function') {
            window._celebrateCorrectAnswer({ xpText: '+10 XP', skipAdvance: true });
        } else {
            document.getElementById("questionCard").classList.add("correct-bg");
...
export function checkOddEvenSelection() {
    ...
    if (isCorrect) {
        ...
        document.getElementById("gameScore").innerText = `${state.score} Correct`;
        // Full celebration suite (sfx + mascot + XP burst + confetti + flash)
        // AND auto-advance — previously this widget showed "Correct!" with no
        // way forward and no celebration sound/cheer.
        if (typeof window._celebrateCorrectAnswer === 'function') {
            window._celebrateCorrectAnswer({ xpText: '+10 XP', skipAdvance: true });
        } else {
            document.getElementById("questionCard").classList.add("correct-bg");
        ...
        if (typeof window.updateSkillProgress === 'function') window.updateSkillProgress(state.skill, true);
        if (typeof window.trackPerformance === 'function') window.trackPerformance(true);
        // Schedule auto-advance to the next question (was missing entirely).
        if (state.mapMode === true && typeof window.recordMapAnswer === 'function') {
            setTimeout(() => window.recordMapAnswer({ correct: true }), 900);
        } else if (typeof window.transitionToNextQuestion === 'function') {
            setTimeout(() => window.transitionToNextQuestion(), 2500);
