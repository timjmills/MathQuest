...
register('fact', {
    ...
    render(p, ctx) {
        if (drawsStack(p, ctx)) return withCue(getCell('stack').render(stackPayload(p), ctx), p);
        return withCue(this.renderFact(p, ctx), p);
    },
    renderFact(p, ctx) {
    ...
    footprint(p, ctx) {
        ...
        if (p.cue) {
            return {
                wMm: Math.ceil(Math.max(vertW, 42)), hMm: factCellHMm(cols, ctx.size) + cueHMm(p),
