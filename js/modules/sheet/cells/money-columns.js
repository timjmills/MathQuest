...
register('money-columns', {
    render(p, ctx) {
        ...
        const opW = g.track;
        // Row captions (critic round 3: the page was bare column sums, "nothing on it is money"):
        // Price / Price / Total, or Paid / Price / Change. Words, never a sign, so they hold at
        // Plain numbers too (SL-9, RP-116).
        const capPt = Math.max(11, zonePt(ctx) + 1);
        const caps = p.op === '-' ? ['Paid', 'Price', 'Change'] : ['Price', 'Price', 'Total'];
        const capW = Math.max(...caps.map((t) => t.length)) * 0.56 * capPt * PT_MM + 2;
        const cap = (t) => `<span style="display:inline-block;width:${L(ctx, capW)};text-align:left;font-size:${P(ctx, capPt)};font-weight:700;`
            + `font-family:'Andika','Open Sans',sans-serif;font-feature-settings:normal;">${esc(t)}</span>`;
        ...
        const lead = (sym, withSign, caption = '') => cap(caption) + (signW ? cellSpan(ctx, signW, withSign ? sign : '') : '') + cellSpan(ctx, opW, sym);
        ...
        const rule = `<div style="width:${L(ctx, ruleW)};margin-left:${L(ctx, capW)};border-top:${L(ctx, 0.79)} solid ${INK};margin-top:${L(ctx, 0.8)};margin-bottom:${L(ctx, 0.8)};"></div>`;
        ...
        const ansRow = `<div style="display:flex;align-items:center;${fontRow}">${cap(caps[2])}${signW ? cellSpan(ctx, signW, '') : ''}${cellSpan(ctx, opW, '')}`
        ...
        return `<div class="tm-cell tm-columns"${isTwin(ctx) ? ' data-mq-tm="1"' : ''}${join} style="color:${INK};font-family:'Andika','Open Sans',sans-serif;`
            ...
            + reg + numRow(lead('', true, caps[0]), A) + numRow(lead(p.op === '-' ? MINUS : '+', false, caps[1]), Bd) + rule + ansRow + `</div></div>`;
    },
    answerKey(p) {
        const res = p.op === '-' ? p.a - p.b : p.a + p.b;
        if (!p.cents) {
